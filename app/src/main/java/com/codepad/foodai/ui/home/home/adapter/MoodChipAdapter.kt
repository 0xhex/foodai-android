...
class MoodChipAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val moodResId = if (isEditing) {
            allMoods[position]
        } else {
            // Convert selected mood string back to resource ID
            val selectedMoodText = selectedMoods.elementAt(position)
            allMoods.find { resId ->
                holder.itemView.context.getString(resId) == selectedMoodText
            } ?: allMoods[0] // Fallback
        }
        holder.bind(moodResId)
    }

    override fun getItemCount(): Int = if (isEditing) allMoods.size else selectedMoods.size
