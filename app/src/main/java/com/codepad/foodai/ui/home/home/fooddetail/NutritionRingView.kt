...
class NutritionRingView @JvmOverloads constructor(
    ...
    val PROTEIN_COLOR = R.color.proteinColor // Protein is red color
    val CARBS_COLOR = R.color.carbsColor // Carbs are orange
    val FATS_COLOR = R.color.fatsColor // Fats are blue
    ...
    private var onCaloriesClickListener: (() -> Unit)? = null
    private var onProteinClickListener: (() -> Unit)? = null
    private var onCarbsClickListener: (() -> Unit)? = null
    private var onFatsClickListener: (() -> Unit)? = null
    ...
    private fun setupClickListeners() {
        ...
        findViewById<View>(R.id.calories_container).setOnClickListener {
            onCaloriesClickListener?.invoke()
        }

        // Protein card click handler 
        proteinCard.setOnClickListener {
            onProteinClickListener?.invoke()
        }

        // Carbs card click handler
        carbsCard.setOnClickListener {
            onCarbsClickListener?.invoke()
        }

        // Fats card click handler
        fatsCard.setOnClickListener {
            onFatsClickListener?.invoke()
        }
    }

    fun setOnCaloriesClickListener(listener: () -> Unit) {
        onCaloriesClickListener = listener
    }

    fun setOnProteinClickListener(listener: () -> Unit) {
        onProteinClickListener = listener
    }

    fun setOnCarbsClickListener(listener: () -> Unit) {
        onCarbsClickListener = listener
    }

    fun setOnFatsClickListener(listener: () -> Unit) {
        onFatsClickListener = listener
    }

    fun updateWithNutritionData(foodDetail: ImageData, shouldAnimate: Boolean = false) {
        ...
        updatePieChart(foodDetail, shouldAnimate)
        ...
        if (shouldAnimate) {
            ...
            // If not animating, just make cards visible immediately
            proteinCard.alpha = 1f
            proteinCard.translationX = 0f
            carbsCard.alpha = 1f
            carbsCard.translationX = 0f
            fatsCard.alpha = 1f
            fatsCard.translationX = 0f
        }
    }

    private fun updatePieChart(foodDetail: ImageData, shouldAnimate: Boolean = false) {
    ...
    private fun runConnectionAnimation() {
        ...
        val initialDelay =
            0L // We use 0 here since we already added the right delay when scheduling this runnable
